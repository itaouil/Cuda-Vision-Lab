...
class MyDecDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> (torch.tensor, dict):
        ...
        img_name = os.path.join(self.root_dir, object_entries.iloc[0, 0])
        image = read_image(img_name + ".png")
...
class MySegDataset(Dataset):
    def __init__(self, base_dir: str, transform: transforms = None):
        ...
        self.img_paths = sorted(self.base_dir.glob(f'image/*.jpg'))
        self.lbl_paths = sorted(self.base_dir.glob(f'target/*png'))
